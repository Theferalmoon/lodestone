...
| `LODESTONE_COMMIT_HASH` | string | Build-time injection of the commit hash into the CLI's `--version` output. If unset, source checkouts fall back to runtime `git rev-parse`; installed packages under `node_modules` use packaged `dist/build-info.json`, then `dev`. |

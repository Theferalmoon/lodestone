...
2. Validate Node.js 20+, npm, and curl.
3. Resolve the pinned `LODESTONE_VERSION`.
4. Download four release tarballs:
   - `lodestone-shared`
   - `lodestone-mcp-server`
   - `lodestone-cli`
   - profile-specific `lodestone-ingest`
5. Verify each tarball against an embedded SHA-256 checksum.
6. Install the tarballs into the target project's `node_modules`.
7. Run `./node_modules/.bin/lodestone init`.
8. Leave no temporary download directory behind.

...
describe("readInstallManifest", () => {
  ...
  it("returns schema-mismatch when schema_version is wrong", () => {
    mkdirSync(path.join(tmp, ".lodestone"), { recursive: true });
    writeFileSync(
  ...
  it("returns ok with parsed manifest on a valid file", () => {
    mkdirSync(path.join(tmp, ".lodestone"), { recursive: true });
    const manifest = {
      schema_version: 1,
    ...
    if (r.ok) expect(r.manifest.schema_version).toBe(1);
  });
